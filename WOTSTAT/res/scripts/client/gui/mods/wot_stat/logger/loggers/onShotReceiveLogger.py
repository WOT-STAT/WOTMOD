...
class OnShotReceiveLogger:
  ...
  def on_health_changed(self, obj, newHealth, oldHealth, attackerID, attackReasonID):
    if obj.id == BigWorld.player().playerVehicleID and \
        attackerID in BigWorld.player().arena.vehicles and \
        BigWorld.player().arena.vehicles[attackerID]['team'] != BigWorld.player().team:
  ...
  def showOwnVehicleHitDirection(self, obj, hitDirYaw, attackerID, damage, crits, isBlocked, isShellHE, damagedID,
                                 attackReasonID):
    if attackerID in BigWorld.player().arena.vehicles and \
        BigWorld.player().arena.vehicles[attackerID]['team'] != BigWorld.player().team:
      print_debug(
