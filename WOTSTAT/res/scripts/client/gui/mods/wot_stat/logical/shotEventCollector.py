...
class ShotEventCollector:
    ...
    class EventCollector:
        ...
        def _shot_damage(self, vehicleID, newHealth, oldHealth):
            ...
            if collection:
                ...
                ammo_bay_destr = SPECIAL_VEHICLE_HEALTH.IS_AMMO_BAY_DESTROYED(newHealth) or SPECIAL_VEHICLE_HEALTH.IS_TURRET_DETACHED(newHealth)
    ...
    def hide_tracer(self, shotID, point):
        ...
        self.append_event(ShotEventCollector.Event(ShotEventCollector.Event.HIDE_TRACER, {'shotID': shotID, 'point': point}))
    ...
    def shot_result(self, vehicleID, flags):
        print_debug('[shot_result] vehicle: %s; direct: %s; ricochet: %s; damage: %s; crits: %s; kill: %s; fire: %s; flags: %d' % (vehicleID,
            ...
            bool(flags & (VHF.MATERIAL_WITH_POSITIVE_DF_PIERCED_BY_PROJECTILE | VHF.MATERIAL_WITH_POSITIVE_DF_PIERCED_BY_EXPLOSION)),
        ...
        self.append_event(ShotEventCollector.Event(ShotEventCollector.Event.SHOT_RESULT, {'vehicleID': vehicleID, 'flags': flags}))
    ...
    def shot_damage(self, vehicleID, newHealth, oldHealth):
        print_debug('[shot_damage] vehicle: %s; damage: %s; kill: %s; ammo_bay_destr: %s; newHealth: %s; oldHealth: %s' % (
            ...
            newHealth <= 0,
            SPECIAL_VEHICLE_HEALTH.IS_AMMO_BAY_DESTROYED(newHealth) or SPECIAL_VEHICLE_HEALTH.IS_TURRET_DETACHED(newHealth),
            newHealth,
            oldHealth
        ))
        self.append_event(ShotEventCollector.Event(ShotEventCollector.Event.SHOT_DAMAGE, {'vehicleID': vehicleID, 'newHealth': newHealth, 'oldHealth': oldHealth}))
    ...
    def fire_damage(self, vehicleID, newHealth, oldHealth):
        ...
        self.append_event(ShotEventCollector.Event(ShotEventCollector.Event.FIRE_DAMAGE, {'vehicleID': vehicleID, 'newHealth': newHealth, 'oldHealth': oldHealth}))
